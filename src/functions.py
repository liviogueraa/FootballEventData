...
def generate_html_report(
    ...
    Parameters:
        ...
        fig_before, fig_after: matplotlib Figure objects (the intact and
            ...
            plot_passing_network_after_removal). They are saved as PNG
            images next to output_path and embedded via <img> tags.
        ...
        reachability_comparison: DataFrame, as returned by
            compare_reachability_before_after
    ...
    reachability_html = _format_df(
        reachability_comparison,
        float_cols=['distance_before', 'distance_after', 'change'],
        bool_cols=['reachable_before', 'reachable_after']
    )
...
<div class="metric-explainer">
    The diagrams below show the team's typical passing structure on the pitch.
    Node color and size reflect each player's <strong>weight per 90 minutes</strong>
    (how much dangerous, progressive passing value they're involved in).
    Edge thickness reflects how strong that specific passing connection is.
    On the right, the playmaker has been removed (man-marked); the dashed red
    circle marks where he used to stand.
</div>
...
<div class="metric-explainer">
    ...
    in the network (by total edge weight), before and after the playmaker
    is removed. Comparing the two lists shows which dangerous lanes
    disappear entirely, and which ones emerge or grow stronger to take
    their place once the team has to circulate the ball without him.
</div>
<div>
...
<h2>5. Forward Reachability</h2>
<div class="metric-explainer">
    This table shows, for each forward, how far (on average) the rest of
    the team is from being able to progress the ball to him, before and
    after the playmaker is removed. "Not reachable" means that, in the
    simulation, no path exists anymore to get the ball to that player at all —
    the strongest possible signal that isolating the playmaker cuts him off.
</div>
{reachability_html}
